...
//        Boolean g1Boolean = false;
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0; j < columns; j++) {
                ...
                if (sheet.getSheetName().contains("送货")){
                    dealDeliveryExcel(cells,cell);
                }
...
//        table.setLockedWidth(true);
    ...
    private void dealDeliveryExcel(LinkedList<PdfPCell> cells, Cell cell) {
        try {
            String cellVal = String.valueOf(getCellValue(cell));
            if ("司机电话".equals(cellVal)||"运输车号".equals(cellVal)){
                for (int k = 0; k < 6 ; k++) {
                    cells.removeLast();
                }
            }
